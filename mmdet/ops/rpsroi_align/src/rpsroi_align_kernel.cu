...
// rpsroi_align:
// 先求得采样点在rroi局部坐标系中的坐标
// 通过坐标变换将其变换到特征图全局坐标系
// 通过双线性插值求得全局坐标上的像素值
// 平均池化
template <typename scalar_t>
__global__ void RPSROIAlignForward(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    const scalar_t* offset_bottom_rois = rois + n * 6;
    // (batch_ind, x_center, y_center, w, h, theta)
    ...
    scalar_t roi_w = offset_bottom_rois[3] * spatial_scale;
    scalar_t roi_h = offset_bottom_rois[4] * spatial_scale;
    scalar_t roi_theta = offset_bottom_rois[5];

    // force malformed rois to be 1x1
    roi_w = max(roi_w, (scalar_t)1.);
    roi_h = max(roi_h, (scalar_t)1.);
    ...
    scalar_t roi_start_h = -roi_h / 2.0;
    scalar_t roi_start_w = -roi_w / 2.0;
    scalar_t cosTheta = cos(roi_theta);
    scalar_t sinTheta = sin(roi_theta);

    const scalar_t sample_count = sample_num_h * sample_num_w;
    scalar_t out_sum = 0.0;
    for (int iy = 0; iy < sample_num_h; iy++) { //e.g., iy = 0,1
      const scalar_t yy = roi_start_h + ph * bin_size_h +
                            (scalar_t)(iy + scalar_t(.5f)) * bin_size_h /
                             (scalar_t)(sample_num_h);  //e.g., 0.5, 1.5
    ...
    top_data[index] = out_sum / sample_count;
    mapping_channel[index] = c;
  }
}

int RPSROIAlignForwardLauncher(const at::Tensor features, const at::Tensor rois,
...
template <typename scalar_t>
__global__ void RPSROIAlignBackward(
  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    const scalar_t diff_val = top_diff[index];
...
int RPSROIAlignBackwardLauncher(const at::Tensor top_grad, const at::Tensor rois,
  ...
  const int output_size = num_rois * out_chn * pooled_h * pooled_w;
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      top_grad.type(), "RPSROIAlignLaucherBackward", ([&] {
