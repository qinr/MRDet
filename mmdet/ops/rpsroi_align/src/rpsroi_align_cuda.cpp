...
int rpsroi_align_forward_cuda(at::Tensor features, at::Tensor rois,
                               int pooled_height, int pooled_width,
                               float spatial_scale, int sample_num,
                               int group_size, int out_channels,
                               at::Tensor output, at::Tensor mapping_channel) {
  ...
  // call the gpu kernel for psroi_Align
  RPSROIAlignForwardLauncher(features, rois, spatial_scale, sample_num, in_channels,
...
int rpsroi_align_backward_cuda(at::Tensor top_grad, at::Tensor rois,
  ...
  RPSROIAlignBackwardLauncher(top_grad, rois, mapping_channel, spatial_scale, sample_num,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &rpsroi_align_forward_cuda, "RPSRoi_align forward (CUDA)");
  m.def("backward", &rpsroi_align_backward_cuda,
        "RPSRoi_align backward (CUDA)");
}
