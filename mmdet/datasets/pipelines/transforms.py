...
class Resize(object):
    ...
    def _random_scale(self, results):
        if 'resize' not in results:
            resize = True if np.random.rand() < self.resize_ratio else False
            results['resize'] = resize
            # results['resize'] = False
        if not results['resize']:
            scale = results['img_shape'][:2]
            scale_idx = 0
        else:
    ...
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            ...
            if key == 'gt_bboxes':
                h = bboxes[:, 3] - bboxes[:, 1]
                w = bboxes[:, 2] - bboxes[:, 0]
                min_w_h = np.minimum(h, w)
                inds = min_w_h > self.small_filter
                results[key] = bboxes[inds]
                results['inds'] = inds
...
@PIPELINES.register_module
class MixUp(object):
    def __init__(self, mixup_ratio=0.5, alpha=1.5, max_bbox_num=2000):
        self.alpha = alpha
    ...
    def __call__(self, results):
        img1, box1, label1, mask1 = [
            results[k] for k in ('img', 'gt_bboxes', 'gt_labels', 'gt_masks')
        ]

        if random.random() < self.mixup_ratio and self.img2 is not None \
                and (len(box1) + len(self.box2)) <= self.max_bbox_num:
            # print("********** start mixup **********")
            ...
            # lambd = random.beta(self.alpha, self.alpha)
            lambd = 0.5
            ...
            mixup_image[:self.img2.shape[0], :self.img2.shape[1], :] += self.img2.astype('float32') * (1. - lambd)
            ...
            mixup_label = np.hstack((label1, self.label2))
            mixup_box = np.vstack((box1, self.box2))
            # mixup_label = np.hstack((label1, self.labels2))
            mixup_mask = np.vstack((mask1, self.mask2))
            # print(self.lambd,mixup_boxes,mixup_label)
            results['img'] = mixup_image
            results['gt_bboxes'] = mixup_box
            results['gt_labels'] = mixup_label
            results['gt_masks'] = mixup_mask
            results['mixup'] = True
            results['mixup_lambd'] = lambd
            results['mixup_num1'] = len(label1)
            results['mixup_num2'] = len(self.label2)
            # cv2.imwrite('./mixup/mixup'+str(mixup_label)+'.jpg',mixup_image)
            # print(mixup_label)
        else:
            results['mixup'] = False
            results['mixup_lambd'] = 0
            results['mixup_num1'] = 0
            results['mixup_num2'] = 0
            # print("********** not mixup **********")
        self.img2 = img1
        self.box2 = box1
        self.label2 = label1
...
class Filter(object):
    ...
    def __call__(self, results):
        gt_bboxes = results['gt_bboxes']
        if len(gt_bboxes) > self.max_bbox_num:
            return None
        else:
            return results